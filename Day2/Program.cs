...
namespace Day2
{
    internal class Program
    {
        ...
        {
            var horribleSpreadsheet = @"86	440	233	83	393	420	228	491	159	13	110	135	97	238	92	396
...
3111	2857	2312	3230	149	3082	408	1148	2428	134	147	620	128	157	492	2879";
            ...
            var lessHorribleSpreadsheet = HorribleSpreadSheetToInts(horribleSpreadsheet);

            var resultPart1 = CalculateChecksum(lessHorribleSpreadsheet);
            var resultPart2 = SumEvenlyDivisibleRows(lessHorribleSpreadsheet);
        }

        private static int SumEvenlyDivisibleRows(IEnumerable<int[]> lessHorribleSpreadsheet)
        {
            // Pretty nasty, must be a better way of doing this.
            var result = 0;
            foreach (var row in lessHorribleSpreadsheet)
            {
        ...
        private static int CalculateChecksum(IEnumerable<int[]> lessHorribleSpreadsheet)
        {
            return lessHorribleSpreadsheet.Sum(x => x.Max() - x.Min());
        }

        private static IEnumerable<int[]> HorribleSpreadSheetToInts(string horribleSpreadsheet)
        {
            return horribleSpreadsheet.Split(new string[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries)
                .Select(x =>
                    x.Split('\t').Select(y => Convert.ToInt32(y)).ToArray()
                    );
        }
    }
}